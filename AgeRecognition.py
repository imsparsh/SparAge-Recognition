...
class AgeRecognition():
    ...
    def train_predict(self, clf, title, X, Y):
        #X = scp.scale(X)
        print("")
...
if __name__ == '__main__':
    ...
    model.train_predict(KNeighborsClassifier(n_neighbors=5, weights='distance'), '5 Neighbors', X, Y)
    model.train_predict(SVC(kernel='linear', C=0.3), 'SVC(Linear)Classifier', X, Y)
    model.train_predict(MLPClassifier(algorithm='adam', alpha=1e-5, hidden_layer_sizes=(200,), activation='relu', random_state=1, learning_rate='constant'), 'MultiLayer BP', X, Y)
    model.train_predict(LogisticRegression(), 'Logistic Regression', X, Y)
